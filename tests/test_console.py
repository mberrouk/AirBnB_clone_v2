#!/usr/bin/python3
"""A unit test module for the CLI.
"""
import json
import sqlalchemy
import unittest
...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        ...
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            cons.onecmd('create City name="Agadir"')
            mdl_id = cout.getvalue().strip()
            clear_stream(cout)
            self.assertIn('City.{}'.format(mdl_id), storage.all().keys())
            cons.onecmd('show City {}'.format(mdl_id))
            self.assertIn("'name': 'Agadir'", cout.getvalue().strip())
            clear_stream(cout)
            cons.onecmd('create User name="Moad" age=25 height=6.2')
            mdl_id = cout.getvalue().strip()
            self.assertIn('User.{}'.format(mdl_id), storage.all().keys())
            clear_stream(cout)
            cons.onecmd('show User {}'.format(mdl_id))
            self.assertIn("'name': 'Moad'", cout.getvalue().strip())
    ...
    def test_db_create(self):
        """Create command with the database storage.
        """
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
    ...
    def test_db_count(self):
        """Count command with the database storage.
        """
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            dbc = MySQLdb.connect(
    ...
    def test_db_show(self):
        """Show command with the database storage.
        """
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            obj = User(email="aga3334@gmail.com", password="152")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            result = cursor.fetchone()
            self.assertTrue(result is not None)
            self.assertIn('aga3334@gmail.com', result)
            self.assertIn('152', result)
            self.assertIn('aga3334@gmail.com', cout.getvalue())
            self.assertIn('152', cout.getvalue())
            cursor.close()
            dbc.close()
